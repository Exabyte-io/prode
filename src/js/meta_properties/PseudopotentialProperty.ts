...
export default class PseudopotentialProperty extends MetaProperty implements FileDataItem {
    ...
    static safelyFilterRawDataBySearchText(
        rawData: PseudopotentialProperty[],
        searchText = "",
        separator = ",",
    ) {
        if (!searchText) return rawData;
        const filteredData: PseudopotentialProperty[] = [];
    ...
    static filterUnique(array: PseudopotentialProperty[]) {
        return uniqBy(array, (item) => item.path);
    }

    // filter unique by apps (assuming that path is always unique)
    static filterUniqueByAppName(array: PseudopotentialProperty[], appName: string) {
        return PseudopotentialProperty.filterUnique(this.filterByAppName(array, appName));
    }
    ...
    static filterByAppName(pseudos: PseudopotentialProperty[], appName: string) {
        return pseudos.filter((pseudo) => pseudo.apps.includes(appName));
    }

    static filterByElements(pseudos: PseudopotentialProperty[], elements: string[]) {
     ...
     */
    static sortPseudosByPattern(pseudos: PseudopotentialProperty[], pattern = "/gbrv/") {
     ...
     */
    static sortByPathVASP(pseudos: PseudopotentialProperty[], version = "5.2") {
        return pseudos.concat([]).sort((a, b) => {
